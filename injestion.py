...
# Reranker is exposed through more than one NVIDIA endpoint depending on model age.
NVIDIA_RERANK_URLS = [
...
# --- Active NVIDIA model defaults (verified against the live API in Aug 2026) ---
# Deprecated IDs such as "meta/llama-4-maverick-17b-128e-instruct",
# "nvidia/llama-3.3-nemotron-super-49b-v1", and "meta/llama-3.1-8b-instruct"
# have reached end-of-life and routinely return 410/404 responses.
...
class NvidiaNIMClient:
    ...
    async def chat_completion(
        ...
        candidates = [model] + [m for m in ACTIVE_CHAT_MODELS if m != model]
            ...
            async with httpx.AsyncClient() as client:
                try:
                    data = await self.post_with_backoff(
                        client, NVIDIA_CHAT_URL, payload, label=f"{label} [{candidate}]", timeout=timeout
                    )
                    try:
                    ...
                    last_error = exc
                    self.status(f"Chat model fallback: {candidate} failed for {label}; trying next model. {exc}")
        raise RuntimeError(f"All chat models failed for {label}: {last_error}")

    async def embed_texts(self, texts: list[str], batch_size: int = 16) -> list[list[float]]:
        if not texts:
            return []

        candidates = [EMBED_MODEL] + [m for m in ACTIVE_EMBED_MODELS if m != EMBED_MODEL]
        last_error: Exception | None = None
        for model_name in candidates:
            ...
            try:
                async with httpx.AsyncClient() as client:
                    for start in range(0, len(texts), batch_size):
                        ...
                        data = await self.post_with_backoff(
                            ...
                            NVIDIA_EMBED_URL,
                            payload,
                            label=f"embedding batch {start // batch_size + 1} [{model_name}]",
                            timeout=120.0,
                        )
                ...
                last_error = exc
                self.status(f"Embedding model fallback: {model_name} failed; trying next model. {exc}")
            ...
            for url in NVIDIA_RERANK_URLS:
                for payload in payload_variants:
                    try:
                        ...
                        last_error = exc
            self.status(
                "NVIDIA reranking is unavailable; using deterministic lexical ranking "
                "so retrieval can continue without dropping evidence."
            )
            return self._local_rerank(query, chunks)

    @staticmethod
    def _local_rerank(
        query: str, chunks: list[DocumentChunk]
    ) -> list[tuple[DocumentChunk, float]]:
        """Rank retrieved chunks locally when the hosted reranker is unavailable."""
        query_terms = {
            term
