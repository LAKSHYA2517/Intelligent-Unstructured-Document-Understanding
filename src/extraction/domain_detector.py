...
class DomainDetector:
    """Detects a document's domain with a single configured-LLM call."""
    ...
    def __init__(self) -> None:
        ...
        logger.info("DomainDetector ready (LLM from config: %s)", self._model)

    def detect(self, chunk_result: ChunkResult) -> DomainResult:
        """Classify a chunked document into one domain.

        Args:
            chunk_result: Stage 3 output; its first 500 words are sampled.

        Returns:
            A :class:`DomainResult`. Empty input short-circuits to ``general``
            with zero confidence and makes no LLM call.
        """
