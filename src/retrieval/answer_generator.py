...
The LLM calls go through the multi-provider cloud router (NVIDIA -> Groq -> Gemini).
"""
...
class AnswerGenerator:
    ...
    Uses the multi-provider cloud LLM router (NVIDIA -> Groq -> Gemini)
    instead of local Ollama for fast, reliable answer generation.
    """

    def __init__(self, llm_client=None) -> None:
        # llm_client kept for backwards-compat but ignored (we use the router)
    ...
    def _generate_multi_hop(
        ...
        """Decompose into a DAG → retrieve and resolve sequentially → synthesise."""
        dag = self._decompose_dag(query)
        ...
        # Execute nodes sequentially (assuming the LLM topologically sorts them in the array)
        for node in dag.get("nodes", []):
            node_id = node.get("id")
            node_query = node.get("query", "")
            
            # Resolve dependencies in the query string and check for adversarial failures
            adversarial_dep = False
            for dep in node.get("depends_on", []):
                if dep in results:
                    if results[dep] == ADVERSARIAL_MESSAGE:
                        adversarial_dep = True
                    node_query = node_query.replace(f"{{{dep}}}", str(results[dep]))
            
            sub_questions.append(node_query)
            
            if adversarial_dep:
                # Skip downstream queries if upstream dependencies fail
                results[node_id] = ADVERSARIAL_MESSAGE
                sub_answers.append(ADVERSARIAL_MESSAGE)
                continue
                
            # Retrieve and generate answer for this specific step
            rr = retrieve_fn(node_query)
            sub = self._generate_standard(node_query, rr, visual)
            
            results[node_id] = sub.answer.strip()
            sub_answers.append(sub.answer.strip())
    ...
    def _decompose_dag(self, query: str) -> dict:
        """One LLM call to split a complex query into a DAG of sub-queries."""
        system_prompt = (
            "You are a query decomposition module.\n"
            "Your goal is to take a complex multi-hop question and break it into a logical "
            "Directed Acyclic Graph (DAG) of sub-queries.\n\n"
            "Output valid JSON exactly in this format:\n"
            "{\n"
            '  "nodes": [\n'
            '    {"id": "step_1", "query": "Find the company that acquired Acme.", "depends_on": []},\n'
            '    {"id": "step_2", "query": "What is the revenue of {step_1}?", "depends_on": ["step_1"]}\n'
            "  ]\n"
            "}"
        )
        raw = self._call_llm(query, system_prompt=system_prompt, json_mode=True)
        try:
            return json.loads(raw)
        except json.JSONDecodeError:
            return {"nodes": [{"id": "step_1", "query": query, "depends_on": []}]}
    ...
    def _call_llm(self, prompt: str, system_prompt: str | None = None, json_mode: bool = False) -> str:
        """Invoke the cloud LLM router and return the response text.

        Uses the native synchronous completion method from the litellm router
        to avoid deadlocking the FastAPI event loop.
        """
        from src.llm_router import generate
        try:
            return generate(prompt, system_prompt=system_prompt, json_mode=json_mode, model="router-llm")
        except Exception as exc:
            logger.warning("Answer LLM call failed: %s", exc)
            return ADVERSARIAL_MESSAGE if not json_mode else "{}"

    def _get_llm(self):
        """Kept for backwards-compat; router is used directly in _call_llm."""
        return None
