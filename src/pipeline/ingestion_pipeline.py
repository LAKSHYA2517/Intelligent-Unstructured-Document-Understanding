...
class IngestionPipeline:
    ...
    def _ingest_document(
        ...
        total = time.time() - doc_start
        timing_summary = " ".join(f"{k}={v:.1f}s" for k, v in sorted(timings.items()))
        logger.info("[%d/%d] '%s' done in %.1fs — %s", index + 1, count, name, total, timing_summary)
