...
import {
  ...
  X, ArrowLeft, Mail, BookOpen, Scale, FileCheck, Copy, XCircle
} from 'lucide-react';
...
const Dashboard = ({ setView }) => {
  ...
  useEffect(() => {
    fetch('http://localhost:8000/files')
      ...
      .then(data => {
        if (data.files) {
          setDocs(data.files.map((f, i) => ({ id: i, name: f.name || f, status: 'Parsed', icon: <FileText size={18} /> })));
        }
      })
  ...
  const handleFileUpload = async (event) => {
    ...
    setDocs(prev => [{ id: tempId, name: file.name, status: 'Uploading...', icon: <FileText size={18} /> }, ...prev]);
        ...
        const pollInterval = setInterval(async () => {
          try {
            ...
            if (statusData.status === 'completed') {
              ...
              setDocs(prev => prev.map(d => d.id === tempId ? { ...d, status: 'Parsed' } : d));
  ...
  const handleSend = async () => {
    ...
    try {
      const res = await fetch('http://localhost:8000/ask', {
        ...
        body: JSON.stringify({ question: userMsg, filename: docs[0]?.name || null })
      });
      const data = await res.json();
      
      setMessages(prev => [...prev, {
        sender: 'ai',
        text: data.answer || "I'm sorry, an error occurred.",
        citations: data.sources?.map((s, i) => ({ id: `c${i}`, label: `[Page ${s.page}]`, preview: s.preview })) || []
      }]);
    } catch (err) {
      ...
      <div className="flex-1 w-full flex gap-2 min-h-0">
        ...
        <GlassCard className="w-1/4 h-full flex flex-col rounded-xl overflow-hidden">
          ...
          <div className="p-4 flex-1 overflow-y-auto">
            {/* Drag & Drop Zone */}
            <label className="border-2 border-dashed border-storm-500/50 rounded-xl p-6 text-center mb-6 hover:bg-white/5 transition-colors cursor-pointer block">
            ...
            {isTyping && (
              <div className="flex justify-start">
                <div className="bg-white/5 border border-white/10 rounded-2xl rounded-bl-none p-4 flex items-center gap-2">
                  <Loader2 size={16} className="animate-spin text-storm-300" />
                  <span className="text-sm text-storm-100/60">Reasoning over knowledge graph...</span>
                </div>
              </div>
            )}
                ...
                >
                  {activeCitation ? (
                    <div className="bg-white/5 border border-white/10 rounded-lg p-4">
                      <div className="flex items-center gap-2 mb-3 text-sm text-storm-300 font-bold uppercase tracking-wider">
                        <FileText size={16} /> Source Context {activeCitation.label}
                      </div>
                      <div className="text-storm-100/80 text-sm leading-relaxed overflow-y-auto whitespace-pre-wrap max-h-[60vh]">
                        {activeCitation.preview || "No preview available for this source."}
                      </div>
                    </div>
                  ) : (
